...
(function () {
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions, getString) {
        ...
        function makeSpritedButtonRow() {
            ...
            buttons.underline = makeButton("wmd-underline-button", getString("underline"), "0px", bindCommand("doUnderline"));
            buttons.strikethrough = makeButton("wmd-strikethrough-button", getString("strikethrough"), "-20px", bindCommand("doStrikethrough"));
            makeSpacer(1);
            ...
            buttons.media = makeButton("wmd-media-button", getString("media"), "-100px", bindCommand(function (chunk, postProcessing) {
                return this.doMedia(chunk, postProcessing);
            }));
            makeSpacer(2);
    ...
    commandProto.doUnderline = function (chunk, postProcessing) {
        return this.doUline(chunk, postProcessing, 2, this.getString("underlineexample"));
    };
    ...
    // nStars: 1 for italics, 2 for bold
    // insertText: If you just click the button without highlighting text, this gets inserted
    commandProto.doUline = function (chunk, postProcessing, nStars, insertText) {
    ...
    // nStars: 1 for italics, 2 for bold
    // insertText: If you just click the button without highlighting text, this gets inserted
    commandProto.doStrike = function (chunk, postProcessing, nStars, insertText) {
    ...
    commandProto.doMedia = function (chunk, postProcessing) {

        chunk.trimWhitespace();
        //chunk.findTags(/\s*!?\[/, /\][ ]?(?:\n[ ]*)?(\[.*?\])?/);
        chunk.findTags(/\s*!?\[!(vimeo|youtube|soundcloud)/, /\][ ]?(?:\n[ ]*)?(\(.*?\))?/);

        var background;

        if (chunk.endTag.length > 1 && chunk.startTag.length > 0) {

            chunk.startTag = chunk.startTag.replace(/\[!(vimeo|youtube|soundcloud)/, "");
            chunk.endTag = "";
            this.addLinkDef(chunk, null);
            ...
            // The function to be executed when you enter a link and press OK or Cancel.
            // Marks up the link and adds the ref.
            var mediaEnteredCallback = function (mediatype, link) {

                background.parentNode.removeChild(background);

                if (mediatype !== null && link !== null) {
                    // (                          $1
                    //     [^\\]                  anything that's not a backslash
                    //     (?:\\\\)*              an even number (this includes zero) of backslashes
                    // )
